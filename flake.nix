  ...
  outputs = publicInputs@{ self, nixpkgs, ... }:
    let
      loadPrivateFlake = path:
        ...
        in builtins.getFlake (builtins.unsafeDiscardStringContext flakePath);
    ...
    in inputs.flake-parts.lib.mkFlake { inherit inputs; } {
      ...
      perSystem = { config, lib, pkgs, self', system, ... }:
        ...
        in {
          ...
          } // packages // (lib.optionalAttrs isLinux (import ./dev/checks.nix {
            inherit self pkgs;
            prefix = "nixos";
          ...
          pre-commit = {
            ...
            settings.hooks.dev-private-narHash = {
              ...
              entry = "sh -c '${
                  lib.getExe pkgs.nix
                } --extra-experimental-features nix-command hash path ./dev/private > ./dev/private.narHash'";
            };
          };
          treefmt = {
